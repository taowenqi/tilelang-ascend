...
from .param import CompileArgs, ProfileArgs, AutotuneResult
from .capture import get_autotune_inputs
...
class AutoTuner:
    ...
    def run(self, warmup: int = 25, rep: int = 100, timeout: int = 30):
        ...
        for i, config_arg in enumerate(config_args):
            ...
            print("len(futures)", len(futures), future)
        ...
        print("<<<< line 539")
        print("len(results_with_configs)", len(results_with_configs))
        ref_latency = None
        ...
        for i in progress_bar:
            ...
            print("best_latency", best_latency)
...
class AutoTuneImpl(Generic[_P, _T]):
    ...
    def get_tunner(self):
        # 如果 jit_impl 没有 func，则直接把函数本身传给 AutoTuner
        if hasattr(self.jit_impl, "func") and self.jit_impl.func is not None:
            kernel_func = self.jit_impl.func
        else:
            print("<<<<< getting a func instead of a jit kernel")
            kernel_func = self.jit_impl  # 普通 function 也可以

        # 生成 autotuner
        autotuner = AutoTuner(kernel_func, configs=self.configs).set_profile_args(
            ...
            out_idx=getattr(self.jit_impl, "out_idx", [0]),
            execution_backend=getattr(self.jit_impl, "execution_backend", "stackvm"),
            target=getattr(self.jit_impl, "target", "npuir"),
            target_host=getattr(self.jit_impl, "target_host", None),
            verbose=getattr(self.jit_impl, "verbose", False),
            pass_configs=getattr(self.jit_impl, "pass_configs", None),
        )
